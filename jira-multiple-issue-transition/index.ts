...
async function start() {
  ...
  if (!targetTransitionName && !targetTransitionId)
    throw new Error(
      "At least one of target-transition or target-transition-id is required"
    );
    ...
    if (targetTransitionName || targetTransitionId) {
      try {
        ...
        if (targetTransitionName && !currentTargetTransitionId) {
          ...
          const currentTransition = response.data.transitions.find(
            (t: any) => t.name === targetTransitionName
          );

          if (!currentTransition) {
            console.error(
              `Transition id of "${targetTransitionName}" for ${issue} not found!\nAvailable transitions are: ${response.data.transitions
        ...
        console.error(
          `Failed to transfer issue ${issue}: ${
            error?.response?.data?.errorMessages?.[0] ?? error
          }`
        );
      }
    }
